...
![IT 审计日志抽查结果](docs/images/readme-sample-analysis.png)

内置脱敏样例包含 8 条日志，页面会展示原始记录、异常数量、涉及记录数、风险筛选和审计关注点，适合在无外部依赖时完整演示。

## 使用流程

1. 设置审计期间并上传 CSV / Excel，或加载内置脱敏样例。
2. 自动识别中英文字段名并转换为标准日志结构。
3. 执行期间、完整性、时间、账号状态、审批冲突与重复记录检查。
4. 按风险等级筛选异常，查看证据、审计关注点和核查建议。
5. 导出异常结果 CSV，供后续抽样与底稿整理使用。
